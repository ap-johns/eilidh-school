# Completion schedule — Maths 7–9 & Science 6–9

**Goal:** finish all seven modules by **Sun 16 Aug 2026** — comfortably ahead of the **1 Sep** deadline (~2 weeks' buffer).

Science 7 and Science 8 are each split in half (a/b) and spread one-per-week across weeks 2–5. No week carries two Maths modules.

| Week | Dates | Due by end of week | Status |
|------|-------|--------------------|--------|
| 1 | Mon 13 – Sun 19 Jul | **Science 6** | ✅ Done |
| 2 | Mon 20 – Sun 26 Jul | **Maths 7** ✅ + **Science 7a** ✅ | ✅ Done |
| 3 | Mon 27 Jul – Sun 2 Aug | **Maths 8** + **Science 7b** | ⬜ To do |
| 4 | Mon 3 – Sun 9 Aug | **Maths 9** + **Science 8a** | ⬜ To do |
| 5 | Mon 10 – Sun 16 Aug | **Science 9** + **Science 8b** | ⬜ To do |

## Notes

- Weeks 2–5 each run at ~1.5 modules (one full + one half) for a gentle, steady curve.
- Maths runs 7 → 8 → 9 in separate weeks (never doubled up).
- Science 7 / 8 halves (a/b) will be split along a natural topic boundary — roughly first half vs. second half of the module's sections.
- Hard deadline: **1 Sep 2026**. Target: **16 Aug 2026**.
